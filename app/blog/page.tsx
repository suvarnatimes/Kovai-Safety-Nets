...
export default async function BlogIndexPage() {
  ...
  const dbPosts = await BlogPost.find({ status: "published" })
    .sort({ createdAt: -1 })
    .lean();
  ...
  return (
    <>
      ...
      {/* Blog Grid (LIGHT) - Reading Live from MongoDB */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {dbPosts.length === 0 ? (
            <div className="text-center py-16 text-slate-500 text-base">
              No published articles found. Create &amp; publish articles from the Admin Panel.
            </div>
          ) : (
